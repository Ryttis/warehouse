<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
            ...
            <div class="flex">
                ...
                @auth()
                    <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                        <x-nav-link :href="route('dashboard', app()->getLocale() )"
                                    :active="request()->routeIs('dashboard')">
                            {{ __('Product List') }}
                        </x-nav-link>
                        <x-nav-link :href="route('color.index', app()->getLocale())"
                                    :active="request()->routeIs('color.index')">
                            {{ __('Color List') }}
                        </x-nav-link>
                        <x-nav-link :href="route('history.index', app()->getLocale())"
        ...
        <!-- Language Setting -->
          @foreach( config('app.available_locales') as $locale )
            <div class="flex items-center text-sm font-medium text-gray-500 hover:text-gray-700 p-0 "
                ...
                <a href="{{ route(\Illuminate\Support\Facades\Route::currentRouteName(), $locale) }}">{{strtoupper($locale)}}</a>
            </div>
          @endforeach
            <!-- Settings Dropdown -->
            @auth
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('dashboard', app()->getLocale())"
                                   :active="request()->routeIs('dashboard')">
                {{ __('Dashboard') }}
